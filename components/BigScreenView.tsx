...
const BigScreenView: React.FC<BigScreenViewProps> = ({ widget, accounts, users, onClose, onSelectAccount, onSelectUser, displayMode }) => {
    ...
    const renderWidget = () => {
        /* Fixed: Use type-safe keys for sorting/rendering based on displayMode */
        const dataKey = displayMode === 'cost' ? 'cost' : 'tokens';
            ...
            case 'account':
                return (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                            layout="vertical"
                            /* Fixed: Use dataKey to sort accounts dynamically */
                            data={[...accounts].sort((a, b) => b[dataKey] - a[dataKey])}
                        ...
                        >
                            ...
                            {/* Fixed: Use dynamic dataKey */}
                            <Bar dataKey={dataKey} fill="#6932D5" barSize={accounts.length <= 10 ? 12 : 20} shape={ <AccessibleBar onBarClick={handleBarClick} ariaLabelGenerator={(p: any) => `Navigate to Account Overview for ${p.name}`} /> } />
                        </BarChart>
                    </ResponsiveContainer>
                );
            case 'user':
                return (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                            layout="vertical"
                            /* Fixed: Use dataKey to sort users dynamically */
                            data={[...users].sort((a, b) => b[dataKey] - a[dataKey])}
                            margin={{ top: 5, right: 30, left: 40, bottom: 30 }}
                            barCategoryGap="10%"
                        >
                            <XAxis type="number" stroke="#5A5A72" fontSize={12} tickLine={false} axisLine={{ stroke: '#E5E5E0' }} label={{ value: displayMode === 'cost' ? 'Cost ($)' : 'Credits', position: 'insideBottom', dy: 15, style: { fill: '#5A5A72', fontSize: 12, fontWeight: 500 } }} />
                            ...
                            <Tooltip cursor={{ fill: 'rgba(105, 50, 213, 0.1)' }} content={<CustomTooltip displayMode={displayMode} />} />
                            {/* Fixed: Use dynamic dataKey */}
                            <Bar dataKey={dataKey} fill="#6932D5" barSize={users.length <= 10 ? 12 : 15} shape={ <AccessibleBar onBarClick={handleUserBarClick} ariaLabelGenerator={(p: any) => `Navigate to User Overview for ${p.name}`} /> } />
                        </BarChart>
                    </ResponsiveContainer>
                );
                            ...
                            <div className="flex items-center justify-center py-4">
                                <div className="grid grid-cols-2 gap-12">
                                    {costBreakdownData.map(item => {
                                        ...
                                        /* Fixed: Changed item.credits to item.tokens as per costBreakdownData definition */
